...
spoiler: 'В этой статье мы разберем основные причины деградации производительности иммутабельных сторов в связке с React и рассмотрим подходы к написанию высоконагруженного кода, включая <strong>неочевидные и многим не знакомые</strong>.'
links:
...
1. В документации Redux не советуют так делать, значит не надо.
    ...
    За свой код отвечает разработчик, а не команда Redux. Mattermost Mobile V1 поплатились за это - пришлось все переписывать (но опять вышло плохо - все таки нужно было не "кровати переставлять (c)").
...
- Учитывать, что значение обновляется сразу, а пропы компонентов только после рендеринга. Лучше искать не по индексу, переданному через `props`, а по id, и предполагать что элемента может не быть (так и без мутабельности стоит делать).
