...
Один из самых важных и проблемных пунктов в работе со сторами, очень сильно влияющий на производительность.

### 3.1. Слишком тяжелые селекторы

Если селекторы слишком много вычисляют, то замедляется каждое изменение состояния стора. Решение - мемоизация селектора, вынос вычислений перед изменения стора, либо после - в компонент.

```ts
const Component = ({id}: Props) => {
  const data = useStore((state) => {
    // Длительные вычисления вызываются на каждое изменение состояния стора,
    // что замедляет каждое его изменение.
...
// Либо делаем вычисление в компоненте через useMemo.
// Хуже тем, что вычисляется каждым компонентом отдельно, и может привести
// к проблеме из следующего пункта, но в определенных случаях допустимо.
...
### 3.2. Слишком общие селекторы

Обратная проблема — подписываться на слишком много данных, делая вычисления в самом компоненте, что приводит к слишком частым ререндерам.

```ts
const ListItem = ({id}: Props) => {
  // На каждое изменение currentItemId перерендерятся все ячейки ListItem.
...
Либо можно почитать [документацию](https://reselect.js.org/api/weakMapMemoize) с другими вариантами настройки LRU или WeakMap кэширования.

### 3.4. Отключаемые подписки

Иногда имеет смысл отключать подписки, прокидывая селектор-заглушку, убирая лишние вычисления селектора и ререндеры.

```ts

const Component = ({enabled}: Props) => {
  // Отключаем подписку с помощью заглушки.
...
### 3.5. Ненужные подписки

Код станет еще эффективнее, если не подписываться на данные, которые не нужны для отрисовки. Например, находятся в обработчиках нажатия.
...
Отличное решение для Zustand / Redux - библиотека [RRC](https://github.com/gentlee/rrc), генерирующая все необходимые функции для кэширования предоставленных асинхронных операций, включая хуки и селекторы. Имеет максимально простой, но гибкий интерфейс, полную типизацию, более 100 юнит тестов, иммутабельный стор на выбор (Redux/Zustand), поддерживает нормализацию, дедупликацию, бесконечную пагинацию, мутабельные коллекции (см. далее) и многое другое. И конечно не использует Immer - производительность в приоритете.
...
Недавно как раз вышло обновление 0.22.2 для библиотеки [RRC](https://github.com/gentlee/rrc), которое дает возможность одной опцией сделать все ее внутренние коллекции мутабельными. Остальной код при этом менять чаще всего не потребуется, так как подписок на ее внутренние коллекции по-хорошему быть не должно. Имеется также бенчмарк, демонстрирующий когда это имеет смысл (кратко и упрощенно - для коллекций > 1000 элементов).

Результаты [замеров](https://github.com/gentlee/rrc/blob/main/scripts/benchmark.mjs) времени добавления элемента в коллекцию в зависимости от размера коллекции, **в микросекундах** (чем меньше, тем лучше):
...
## 9. Требовательные к отзывчивости интерфейса операции
