...
shortTitle: 'ООП - <strong>худшее</strong> из программирования'
date: '2025-02-24'
cover: 'cover-ru.webp'
...
- Метод — функция, являющаяся частью класса. Методы экземпляра (не статические) имеют ссылку на сам объект (`this`, `self`), со всеми его данными и методами, которая фактически является неявным первым аргументом.
...
**Во-первых**, метод намертво приколочен к типу своего неявного аргумента — `this`, которым является `User`. Он зависит не от интерфейса, а от конкретного класса. Отсюда мы имеем сразу несколько проблем:
